...
export function ImportPage() {
  ...
  async function handlePropagate() {
    await propagate(storeId, validPayments)
    refreshBackendPayments(storeId)
  }
    ...
    <div className="import-page">
      <header className="page-header">
        <div>
          ...
          <p className="subtitle">
            Edit the sample request JSON, send a single payment to the remote system, or upload a
            CSV notebook and submit an async batch to the coffee-shop backend.
          </p>
        </div>
      </header>
      ...
      {batch && (
        <div className="actions">
          ...
          >
            {propagating ? 'Submitting batch…' : `Submit ${validPayments.length} payments (async)`}
          </button>
          <button
            type="button"
            className="secondary"
